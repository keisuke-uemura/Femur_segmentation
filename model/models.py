...
from tensorflow.keras.layers import Activation, Dropout, ZeroPadding2D, Lambda
...
def UpBlockLayers(h_prev, h_down, n_channels, kernel_size):
    ...
    h_upconv = Conv2DTranspose(h_conv.shape[CHANNEL_AXIS],
                    kernel_size,
                    strides=POOL_SIZE,
...
def ConcatenateLayers(x1, x2):
    dx = (x1.shape[ROW_AXIS] - x2.shape[ROW_AXIS]) / 2
    dy = (x1.shape[COL_AXIS] - x2.shape[COL_AXIS]) / 2
